(function() {
  ...
  //
  // Your library not in here? Fine. Just do "oo.query = <your function here>"
  var query = self.Sizzle     // Sizzle - sizzlejs.com
              ...
              || function() {
                oo.error(Error('No query method found.  Maybe consider including Sizzle(http://sizzlejs.com/)?'));
              };
  ...
  *
  * Iterations can be interrupted by throwing oo.STOP (Synonomous with
  ...
  */
  ...
  function $(el) {
    ...
    //if (el) el = el._ooEl || (el._ooEl = new Element(el));
    return el;
  }
  ...
  function clear(el) {
    oo.each(el, function(cel) {el.removeChild(cel);}, el);
    return el;
  }
  ...
  function animate(params) {
    ...
    if (!o || !params.onProgress) oo.error(Error('Invalid params properties'));
  ...
  var Template = oo.createClass({
    $byId: {},

    initialize: function(el) {
      el = this.el = oo.$(el);
      if (!el) oo.error(Error('Element,"' + el + '", not found'));
  ...
  if (!self.console) {
    self.console = {};
    oo.each('assert count debug dir dirxml error group groupEnd info log profile profileEnd profiles time timeEnd trace warn'.split(/\s+/), function(k) {console[k] = oo.nilf;});
  };
})();
